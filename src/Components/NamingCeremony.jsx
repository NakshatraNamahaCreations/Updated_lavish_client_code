...
const NamingCeremony = () => {
  ...
  const fetchSubSubcategories = async () => {
    if (!subcat_id) return;
    try {
      const res = await getAuthAxios().get(`subsubcategories/subcategory/${subcat_id}`);
      setSubSubCategories(res.data.data);
    } catch (err) {
  ...
  const fetchServices = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/services/filter/${subcat_id}`);
      const data = await res.json();

      if (!res.ok || !data.success) throw new Error("Failed to fetch services");
    ...
    <div className='lg:py-24 md:pt-20 pt-32 p-3 mx-auto'>
      ...
      <div className="grid grid-cols-2 gap-x-8 md:gap-y-14 gap-y-5 md:place-items-center lg:mt-20 mt-10">
        {subSubCategories.map((item) => (
          <div className="relative" key={item._id}>
            <Link to={`/service/${item._id}`}>
              <img src={`http://localhost:5000/images/${item.image}`} alt={item.subSubCategory} className="rounded-3xl w-[500px]" />
            </Link>
            <p className="text-primary pt-4 md:text-3xl text-xl text-center font-medium carter">
              {item.subSubCategory}
            </p>
