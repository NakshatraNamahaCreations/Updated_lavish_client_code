...
const Checkout = () => {
    ...
    const applyCoupon = (couponCode) => {
        ...
        if (couponCode) {
            ...
            const couponData = coupons.find(coupon => coupon.couponCode === couponCode);
            if (couponData) {
    ...
    useEffect(() => {
        if (selectedCoupon && selectedPayPercentage === '100' && selectedCouponData) {
            // Recalculate discount when subtotal changes
            const discount = Math.round(subTotal * (selectedCouponData.discount / 100));
            dispatch(setCouponDiscount(discount));
        } else if (!selectedCoupon) {
            dispatch(setCouponDiscount(0));
        }
    }, [selectedCoupon, selectedPayPercentage, subTotal, selectedCouponData]);
    ...
    const handleProceedToPay = async () => {
        try {
            ...
            const orderData = {
                ...
                ...(currentOrder.occasion === 'others' && { otherOccasion: currentOrder.otherOccasion }),
                ...(currentOrder.decorLocation === 'others' && { otherDecorLocation: currentOrder.otherDecorLocation }),
            };
                            ...
                            <form className='mt-2 py-3 '>
                                ...
                                <div className='grid grid-cols-2 gap-4'>
                                    <div>
                                        ...
                                        {currentOrder.occasion === "others" && <input type="text" onChange={e => dispatch(setOtherOcassion(e.target.value))} placeholder='Enter the occasion' value={currentOrder.otherOccasion} className='w-full border p-2 my-2 text-sm border-gray-300   rounded-md' />}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            onChange={e => dispatch(setDecorLocation(e.target.value))}
                                        >
                                            ...
                                            <option value="banquetHall" className='text-black'>Banquet Hall</option>
                                            <option value="outdoorGarden" className='text-black'>Outdoor Garden</option>
                                            <option value="terrace" className='text-black'>Terrace</option>
                                            <option value="hotelRoom" className='text-black'>Hotel Room</option>
                                            <option value="others" className='text-black'>Others</option>
                                        </select>
                                        {currentOrder.decorLocation === "others" && (
                                            <input
                                                type="text"
                                                onChange={e => dispatch(setOtherDecorLocation(e.target.value))} // ✅ Correct for location
                                ...
                                <div className='border-b-2 border-dashed border-gray-500 text-gray-500  bg-white poppins p-3 space-y-2'>
                                    ...
                                    {selectedPayPercentage === "100" && (
                                        selectedCoupon !== "" ? (
                                            <div className={`flex gap-4 text-lg ${selectedPayPercentage === "50" ? "text-gray-400" : "text-black"}`}>
                                                <div className='w-full'>
                                                    <div className='w-full flex items-center'>
                                                        <p className="text-sm">Coupon {selectedCoupon} ({selectedCouponData?.discount}% OFF)</p>
                                                        <button onClick={handleRemoveCoupon}><MdDelete className='cursor-pointer' size={20} /></button>
                                                    </div>
                                            ...
                                            <div>
                                                <div className='flex items-center gap-4 md:mx-4 md:px-4 px-2 py-2 rounded-md text-black bg-purple-400 cursor-pointer'
