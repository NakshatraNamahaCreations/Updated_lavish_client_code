...
function ServicedetailsSlider() {
    ...
    return (
        <div className="slider-container py-1 my-10">
            <Slider {...settings}>
                {images.map((item, idx) => (
                    <div className="text-center flex  justify-center items-center" key={idx}>
                        <img src={barbieTheme} alt="barbie theme" className="w-[70%] mx-auto  rounded-lg object-cover" />
                        <p className="md:text-lg text-2xl md:py-1 py-2">Pink roses bouquets</p>
                        <div className="w-[60%] mx-auto text-xl flex items-center justify-between  ">
                            <p>Rs. 649</p>
                            <button className="bg-[#AA6300] text-white  px-3 rounded">ADD</button>
                        </div>
                    </div>
                ))}
            </Slider>
